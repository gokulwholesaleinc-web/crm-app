...
const ACTION_COLORS: Record<string, { bg: string; icon: string }> = {
  create: { bg: 'bg-green-100 dark:bg-green-900/30', icon: 'text-green-600 dark:text-green-400' },
  update: { bg: 'bg-blue-100 dark:bg-blue-900/30', icon: 'text-blue-600 dark:text-blue-400' },
  delete: { bg: 'bg-red-100 dark:bg-red-900/30', icon: 'text-red-600 dark:text-red-400' },
};
...
function AuditEntry({ entry }: { entry: AuditLogEntry }) {
  ...
  return (
    <li className="relative pb-8 last:pb-0">
      ...
      <span
        className="absolute left-4 top-8 -ml-px h-full w-0.5 bg-gray-200 dark:bg-gray-700 last:hidden"
        aria-hidden="true"
      />
      ...
      <div className="relative flex items-start space-x-3">
        ...
        <div className="relative">
          <div
            className={`h-8 w-8 rounded-full ${colors.bg} flex items-center justify-center ring-4 ring-white dark:ring-gray-800`}
          >
            {entry.action === 'create' ? (
              <svg
          ...
          {hasChanges && (
            <div className="mt-1">
              <button
                ...
                className="text-xs text-primary-600 dark:text-primary-400 hover:text-primary-500 dark:hover:text-primary-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 rounded"
              >
                {isExpanded
                  ? 'Hide changes'
            ...
            {totalPages > 1 && (
              <div className="mt-6 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 pt-4">
                <button
                  ...
                  disabled={page <= 1}
                  className="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 rounded px-2 py-1"
                >
                  Previous
                </button>
