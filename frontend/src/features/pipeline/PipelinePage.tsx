...
// ---------------------------------------------------------------------------
// Drag ID helpers — encode entity type + id + source stage into one string
// ---------------------------------------------------------------------------

function encodeLeadDragId(leadId: number, stageId: number) {
  return `lead:${leadId}:${stageId}`;
}

function encodeOppDragId(oppId: number, stageId: number) {
  return `opp:${oppId}:${stageId}`;
}

function parseLeadDragId(id: string): { leadId: number; stageId: number } | null {
  const m = id.match(/^lead:(\d+):(\d+)$/);
  if (!m) return null;
  return { leadId: Number(m[1]), stageId: Number(m[2]) };
}

function parseOppDragId(id: string): { oppId: number; stageId: number } | null {
...
// ---------------------------------------------------------------------------
// Overlay cards shown under the pointer while dragging
// ---------------------------------------------------------------------------

function LeadDragOverlay({ lead }: { lead: KanbanLead }) {
  return (
...
function OppDragOverlay({ opp }: { opp: KanbanOpportunity }) {
  return (
