...
export function SortableOppCard({
  ...
  // dnd-kit's listeners include onKeyDown that handles Space to start a drag.
  // We layer Enter (activate the card) on top of that without overwriting it.
  const dndOnKeyDown = listeners?.onKeyDown as ((e: React.KeyboardEvent) => void) | undefined;
  ...
  return (
    <div
      ...
      className="block w-full text-left bg-emerald-50 dark:bg-emerald-900/20 rounded-lg shadow-sm border border-emerald-200 dark:border-emerald-800 p-3 hover:shadow-md transition-shadow duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 cursor-pointer"
    >
