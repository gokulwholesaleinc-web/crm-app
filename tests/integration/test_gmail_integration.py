...
class TestFindThreadContext:
    """_find_thread_context should return the most recent message on the
    thread — inbound or outbound — so replies from the CRM land in the
    correct Gmail thread with correct RFC-822 In-Reply-To headers."""

    @pytest.mark.asyncio
    async def test_returns_none_when_no_prior_messages(self, db_session, contact):
    ...
    @pytest.mark.asyncio
    async def test_returns_outbound_when_only_outbound_exists(
        self, db_session, test_user, contact
    ):
        from src.email.service import _find_thread_context

        outbound = EmailQueue(
            to_email=contact.email,
            from_email="giancarlo@linkcreative.com",
            subject="Hi",
            body="Hello",
            status="sent",
            sent_via="gmail",
            thread_id="thread-outbound-1",
            message_id="<outbound-1@linkcreative.com>",
            entity_type="contacts",
            entity_id=contact.id,
            sent_by_id=test_user.id,
            sent_at=datetime(2026, 4, 17, 10, 0, tzinfo=timezone.utc),
        )
    ...
    @pytest.mark.asyncio
    async def test_returns_inbound_when_only_inbound_exists(
        self, db_session, contact
    ):
        """Reply-to-first-touch-inbound case — previously unthreaded."""
        from src.email.service import _find_thread_context

        inbound = InboundEmail(
            resend_email_id="gmail:inbound-1",
            ...
            entity_type="contacts",
            entity_id=contact.id,
            received_at=datetime(2026, 4, 17, 10, 0, tzinfo=timezone.utc),
        )
    ...
    @pytest.mark.asyncio
    async def test_returns_newest_when_both_exist(
        self, db_session, test_user, contact
    ):
        from src.email.service import _find_thread_context

        older_outbound = EmailQueue(
            to_email=contact.email,
            from_email="giancarlo@linkcreative.com",
            subject="Older",
            body="Older",
            ...
            message_id="<older@linkcreative.com>",
            entity_type="contacts",
            entity_id=contact.id,
            sent_by_id=test_user.id,
            sent_at=datetime(2026, 4, 17, 9, 0, tzinfo=timezone.utc),
            created_at=datetime(2026, 4, 17, 9, 0, tzinfo=timezone.utc),
        )
        newer_inbound = InboundEmail(
            resend_email_id="gmail:newer",
            from_email=contact.email,
            to_email="giancarlo@linkcreative.com",
            subject="Newer reply",
            ...
            entity_type="contacts",
            entity_id=contact.id,
            received_at=datetime(2026, 4, 17, 12, 0, tzinfo=timezone.utc),
        )
        db_session.add_all([older_outbound, newer_inbound])
        await db_session.flush()
    ...
    @pytest.mark.asyncio
    async def test_does_not_match_other_contacts(
        self, db_session, test_user, contact
    ):
        """Threading must be scoped by entity_id — another contact's thread
        must not leak into this contact's reply."""
        ...
        other_thread = InboundEmail(
            resend_email_id="gmail:other",
            from_email=other.email,
            to_email="giancarlo@linkcreative.com",
