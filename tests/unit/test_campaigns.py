...
class TestCampaignsList:
    ...
    async def test_list_campaigns_empty(
        ...
        response = await client.get("/api/campaigns", headers=admin_auth_headers)
...
class TestCampaignOwnerAssignment:
    """Tests for campaign owner_id auto-assignment on creation."""

    @pytest.mark.asyncio
    async def test_create_campaign_sets_owner_to_current_user(
        self,
        client: AsyncClient,
        db_session: AsyncSession,
        admin_auth_headers: dict,
        ...
        """Test that creating a campaign without owner_id sets it to current user."""
        response = await client.post(
            "/api/campaigns",
            headers=admin_auth_headers,
            json={
                "name": "Auto-Owned Campaign",
                "campaign_type": "email",
            },
        )
        ...
        assert data["owner_id"] == test_admin_user.id
