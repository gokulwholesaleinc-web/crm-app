"""Idempotent seed for Lorenzo's 6 Google Forms as onboarding templates (P4).

Lorenzo's intake forms (CLIENT_ONBOARDING_QUESTIONNAIRE_SPEC.md §1) are rebuilt
here as ``OnboardingTemplate`` DATA — the two ``upload-gated`` forms (1, 3) split
into a ``questionnaire`` doc + an ``upload_request`` doc because the questionnaire
kind does not accept ``file_upload`` (§B.3). Eight templates total.
...
service_tag decisions (§C.1 — null = universal/always-offered):
  ...
  * Form 6 "Link Live — Guest Intake"          → ``service_tag="link_live"``.
...
# --------------------------------------------------------------------------
# The 8 template definitions (the 2 mixed forms split into Q + upload docs).
# --------------------------------------------------------------------------
...
def onboarding_template_specs() -> list[dict]:
    """The 8 seed-template specs in stable order (pure data; no DB access).
...
async def seed_onboarding_templates(db: AsyncSession) -> list[OnboardingTemplate]:
    """UPSERT the 8 onboarding templates idempotently, keyed on ``name``.
